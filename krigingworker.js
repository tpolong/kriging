onmessage = function(e) {
    ...
    var pm = e.data.pm;
    var csvX = e.data.csvX;
    var csvY = e.data.csvY;
    var length = data.length;
    //var caculate = e.data[1];
    //console.log(pm, csvX, csvY)
    console.log(length)
    console.time("k1")
    var result = e.data.result;
    var gridPoint = new Float32Array(result);
    ...
    for (var i = 0; i <= length - 1; i++) {
        ...
        var krigingValue = getKring(data[i][0], data[i][1], pm, csvX, csvY);
        gridPoint[i*3+0]=(data[i][0]);
        gridPoint[i*3+1]=(data[i][1]);
        gridPoint[i*3+2]=(krigingValue);
        //console.log(krigingValue);
    }
    //console.log(ArrayBuffer.prototype.slice.call(gridPoint));
    //result = gridPoint;
    //result = Array.prototype.slice.call(new Float64Array(gridPoint));
    //postMessage(result);
    postMessage({
        values:data, 
        pm:pm, 
...
function getKring(xk, yk, pm, csvX, csvY) {
    var t = pm;
    var x = csvX;
    var y = csvY;
    var model = "exponential"; //"spherical"; //"gaussian"; //"exponential";
    var sigma2 = 0,
        alpha = 100;
